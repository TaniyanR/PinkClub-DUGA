# PinkClub DUGA

DUGAの商品情報を活用し、商品検索・ランキング・SEO・アクセス解析・管理機能を備えたアフィリエイト商品サイトを構築するプロジェクトです。

## 現在の状態

企画・仕様整理段階です。  
現時点ではアプリケーションコードを実装していません。

## 開発方針

既存の [PinkClub FANZA](https://github.com/TaniyanR/PinkClub-FANZA) を機能面の基準とし、同等規模のDUGA対応サイトを目指します。

ただし、GitHubのフォークは使用せず、独立したリポジトリとして開発します。FANZA固有のAPI仕様や名称をそのまま流用せず、DUGAの提供仕様を確認したうえで個別に設計・実装します。

## 実装予定の主な機能

### 公開サイト

- 商品一覧・商品詳細
- キーワード検索
- ジャンル・出演者・メーカー・シリーズなどの分類ページ
- 新着・おすすめ・人気ランキング
- 関連商品の表示
- サンプル画像・サンプル動画への対応
- パンくず、canonical、OGP、構造化データ
- sitemap.xml・robots.txt
- PC・スマートフォン対応

### 管理画面

- ログイン認証
- DUGA API設定
- API接続テスト・商品取得
- 自動取得設定と実行履歴
- 商品・分類データ管理
- サイト基本設定
- デザイン・広告設定
- 固定ページ管理
- 相互リンク・RSS管理
- アクセス解析
- アカウント設定
- データベースのバックアップ

### 運用・品質

- cronによる定期取得
- APIデータの重複排除と更新
- 多重実行防止
- キャッシュとインデックスによる軽量化
- CSRF・XSS・セッションなどの基本的なセキュリティ対策
- エラーログ・API実行ログ
- 初回セットアップとデータベース自動構築

## 使用予定技術

- PHP
- MySQL / MariaDB
- JavaScript
- HTML / CSS
- PDO
- フレームワークなし

## 重要な前提

- PinkClub FANZAの既存機能を参考にする
- DUGA固有のAPI仕様と利用規約を優先する
- APIキーやパスワードなどの秘密情報をGitへ保存しない
- 外部画像・動画の保存や利用方法はDUGAの提供条件に従う
- PC・スマートフォンの両方で使いやすくする
- 軽量で一般的なレンタルサーバーに設置しやすい構成にする
- 実装は小さな作業単位に分け、作業ブランチとDraft PRで進める
- ユーザー確認前にmainへマージしない

## 開発の流れ

1. DUGA APIの仕様・利用条件を調査
2. PinkClub FANZAから引き継ぐ機能を整理
3. DUGAの商品項目とデータベース項目を対応付け
4. 初期セットアップと共通基盤を作成
5. API取得・保存・自動更新を実装
6. 公開画面と管理画面をDUGA向けに実装
7. SEO・アクセス解析・周辺機能を実装
8. セキュリティ・速度・総合動作を確認

## 開発管理

作業内容と進行状況は、このリポジトリのIssuesで管理します。

## 関連プロジェクト

- [PinkClub FANZA](https://github.com/TaniyanR/PinkClub-FANZA)
