...
$makerNameSql = db_column_exists('item_makers', 'item_id')
    ? "SELECT im.maker_name FROM item_makers im INNER JOIN makers m ON m.id = :id AND im.duga_id = m.duga_id WHERE TRIM(COALESCE(im.maker_name, '')) <> '' GROUP BY im.maker_name ORDER BY COUNT(*) DESC, im.maker_name ASC LIMIT 1"
...
$title = $makerName;
$pageDescription = mb_strimwidth($makerName . 'の作品一覧。DUGAで販売中の最新作・人気作品を紹介。', 0, 150, '…', 'UTF-8');
