...
function recommendation_relation_rows(string $table, string $column, array $ids): array
{
    ...
    $allowed = [
        'item_actresses' => ['column' => 'actress_id', 'master' => 'actresses'],
        'item_genres' => ['column' => 'genre_id', 'master' => 'genres'],
        'item_makers' => ['column' => 'maker_id', 'master' => 'makers'],
        'item_series' => ['column' => 'series_id', 'master' => 'series_master'],
    ];
    if (!isset($allowed[$table]) || $allowed[$table]['column'] !== $column) {
        return [];
    }
    ...
    try {
        $master = $allowed[$table]['master'];
        $stmt = db()->prepare(
            'SELECT relations.item_id, COUNT(*) AS matches' .
            ' FROM ' . $table . ' relations' .
            ' INNER JOIN ' . $master . ' master ON master.duga_id = relations.duga_id' .
            ' WHERE master.id IN (' . $in . ') AND relations.item_id IS NOT NULL' .
            ' GROUP BY relations.item_id LIMIT 300'
        );
        $stmt->execute($params);
