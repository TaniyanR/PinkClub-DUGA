...
function pick_full_package_image(array $item): string
{
    if (function_exists('pcf_item_image')) {
        $candidate = trim((string)pcf_item_image($item));
        if ($candidate !== '') {
            return $candidate;
        }
    }
    ...
    if ($itemCount > 0) {
        ...
        if (db_table_exists($pdo, 'genres') && db_table_exists($pdo, 'item_genres')) {
            ...
            if ($genreCandidates === []) {
                ...
                if ($genreCandidates === []) {
                    $genreCandidates = query_all_safe($pdo, 'SELECT g.id,g.name,COUNT(*) AS item_count FROM genres g INNER JOIN item_genres ig ON ig.duga_id = g.duga_id GROUP BY g.id,g.name HAVING COUNT(*) > 0 ORDER BY item_count DESC,g.id DESC LIMIT 3');
                }
            }
            foreach (array_slice($genreCandidates, 0, 3) as $index => $genre) {
                $genreItems = [];
                foreach ([
                    'SELECT i.id,i.content_id,i.title,i.image_small,i.image_large,i.image_list,i.raw_json,i.affiliate_url,i.sample_movie_url_720,i.sample_movie_url_644,i.sample_movie_url_560,i.sample_movie_url_476,i.release_date,i.updated_at FROM items i INNER JOIN item_genres ig ON ig.item_id = i.id INNER JOIN genres g ON g.duga_id = ig.duga_id WHERE g.id = :id ORDER BY i.view_count DESC, i.release_date DESC, i.updated_at DESC, i.id DESC LIMIT 120',
                    'SELECT i.id,i.content_id,i.title,i.image_small,i.image_large,i.image_list,i.raw_json,i.affiliate_url,i.sample_movie_url_720,i.sample_movie_url_644,i.sample_movie_url_560,i.sample_movie_url_476,i.release_date,i.updated_at FROM items i INNER JOIN item_genres ig ON ig.item_id = i.id INNER JOIN genres g ON g.duga_id = ig.duga_id WHERE g.id = :id ORDER BY i.release_date DESC, i.updated_at DESC, i.id DESC LIMIT 120',
                ] as $genreSql) {
        ...
        if (db_table_exists($pdo, 'item_series') && (db_table_exists($pdo, 'series') || db_table_exists($pdo, 'series_master'))) {
            ...
            if ($seriesCandidates === [] && db_table_exists($pdo, 'series_master')) {
                $seriesCandidates = query_all_safe($pdo, 'SELECT s.id,s.name,COUNT(isr.id) AS item_count FROM series_master s INNER JOIN item_series isr ON isr.duga_id = s.duga_id GROUP BY s.id,s.name HAVING COUNT(isr.id) > 0 ORDER BY item_count DESC,s.id DESC LIMIT 120');
            }
            if ($seriesCandidates !== []) {
                $seriesCandidates = seeded_shuffle($seriesCandidates, $seedBase + 40);
                $picked = $seriesCandidates[0];
                $seriesItems = query_all_safe($pdo, 'SELECT i.id,i.content_id,i.title,i.image_small,i.image_large,i.image_list,i.raw_json,i.affiliate_url,i.sample_movie_url_720,i.sample_movie_url_644,i.sample_movie_url_560,i.sample_movie_url_476,i.release_date,i.updated_at FROM items i INNER JOIN item_series isr ON isr.item_id = i.id INNER JOIN series_master s ON s.duga_id = isr.duga_id WHERE s.id = :id ORDER BY i.release_date DESC, i.updated_at DESC, i.id DESC LIMIT 120', [':id' => (int)$picked['id']]);
        ...
        if (db_table_exists($pdo, 'makers') && db_table_exists($pdo, 'item_makers')) {
            ...
            if ($makerCandidates === []) {
                $makerCandidates = query_all_safe($pdo, 'SELECT m.id,m.name,COUNT(im.id) AS item_count FROM makers m INNER JOIN item_makers im ON im.duga_id = m.duga_id GROUP BY m.id,m.name HAVING COUNT(im.id) > 0 ORDER BY item_count DESC,m.id DESC LIMIT 120');
            }
            if ($makerCandidates !== []) {
                $makerCandidates = seeded_shuffle($makerCandidates, $seedBase + 50);
                $picked = $makerCandidates[0];
                $makerItems = query_all_safe($pdo, 'SELECT i.id,i.content_id,i.title,i.image_small,i.image_large,i.image_list,i.raw_json,i.affiliate_url,i.sample_movie_url_720,i.sample_movie_url_644,i.sample_movie_url_560,i.sample_movie_url_476,i.release_date,i.updated_at FROM items i INNER JOIN item_makers im ON im.item_id = i.id INNER JOIN makers m ON m.duga_id = im.duga_id WHERE m.id = :id ORDER BY i.release_date DESC, i.updated_at DESC, i.id DESC LIMIT 120', [':id' => (int)$picked['id']]);
        ...
        if (db_table_exists($pdo, 'authors') && db_table_exists($pdo, 'item_authors')) {
            $authorCandidates = $pdo->query('SELECT a.id,a.name,COUNT(ia.id) AS item_count FROM authors a INNER JOIN item_authors ia ON ia.duga_id = a.duga_id GROUP BY a.id,a.name HAVING COUNT(ia.id) > 0 ORDER BY item_count DESC,a.id DESC LIMIT 120')->fetchAll();
            if ($authorCandidates !== []) {
                ...
                $stmt = $pdo->prepare(
                    'SELECT i.id,i.content_id,i.title,i.image_small,i.image_large,i.image_list,i.raw_json,i.affiliate_url,i.sample_movie_url_720,i.sample_movie_url_644,i.sample_movie_url_560,i.sample_movie_url_476,i.release_date,i.updated_at
                     ...
                     INNER JOIN authors a ON a.duga_id = ia.duga_id
                     WHERE a.id = :id
