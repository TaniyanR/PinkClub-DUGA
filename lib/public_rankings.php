...
function pcf_public_ranking_sql(string $type, string $scoreSql, int $limit): string
{
    ...
    if (isset($config[$type])) {
        ...
        return 'SELECT m.id, m.duga_id, m.name,
                ...
                INNER JOIN ' . $master . ' m ON m.duga_id = r.duga_id
                GROUP BY m.id, m.duga_id, m.name
                ORDER BY access_count DESC, out_click_count DESC, m.id DESC
                LIMIT ' . $limit;
    }

    return 'SELECT COALESCE(NULLIF(il.duga_id, ""), il.label_name) AS id,
            ...
            GROUP BY COALESCE(NULLIF(il.duga_id, ""), il.label_name), il.label_name
